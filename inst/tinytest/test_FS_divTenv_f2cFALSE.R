...
set.seed(123)
slc <- FS(mod, select = "e", consider = consider, 
          permutations = 999, factor2categories = f2c)
...
set.seed(123)
an1 <- anova(mod1$RDAonEnv, permutations = 999)
...
set.seed(123)
an2 <- anova(mod2$RDAonEnv, permutations = 999)
...
set.seed(123)
an3 <- anova(mod3$RDAonEnv, permutations = 999)
expect_equivalent(c(an1$`Pr(>F)`[1], 
                    an2$`Pr(>F)`[1],
                    an3$`Pr(>F)`[1]),
