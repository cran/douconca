...
which is also a three-tables method. RLQ is available in the R-package `ade4`
[@dray2007ade4; @thioulouse2018multivariate].
...
[@Braak2026;@BraakRossum2025;@Braak2018dcCA;@kleyer2012assessing].   
...
The value of dc-CA compared to RLQ, GLMM3 and CWM-regression
is discussed, with examples, in ter Braak and van Rossum (2025) and
ter Braak (2026). 				  
...
those in the `vegan` package [@oksanen_vegan2024] on to which `douconca` is
based. Because of the `formula`-interface, names of trait and environmental
variables, and of levels of factors may need to be changed to syntactically 
valid names, e.g. by `make.names` or `janitor::clean_names`.
# Double constained correspondence analysis

As RLQ, dc-CA seeks for an ordination (i.e. a low-dimensional representation of)
of the multi-trait, multi-environment relationships, but dc-CA differs from RLQ 
in that dc-CA is based on regression with the traits and environmental variables 
as predictors, whereas RLQ is based on co-variance.
...
whereas RLQ does not. Moreover, dc-CA is more robust and, with N2-based 
pre-processing, equally powerfull as fourth-corner
generalized linear latent variable models (GLLVM) [@Braak2026].
...
A full-length treatment of dc-CA, with examples, is given in ter Braak and van Rossum (2025).																	 
...
and four ecological traits (Ellenberg indicator values for moisture (F), 
...
environmental values are fitted species-niche centroids, as is clearer from
the statements `fitted(mod, type = "CWM")` and `fitted(mod, type = "SNC")`,
respectively.
...
`coef(mod_LDMC,  type = "env2traits",normed = FALSE)`
which give the same values as `coef(lmLDMC)[-1]`.

## Restrictions on names of variables and levels of factors

As said, the `douconca` package has a `formula`-interface to specify the dc-CA
model. By consequence, there are restriction on the names of traits and
environmental variables. For example these should not include ":" or "/", as 
these symbols are special in formulas. The same applies to names of levels of
factors as factors are transformed to indicator matrices using a formula. 
